...
N2QL queries that turn the [detection catalog](../detections/detection-catalog.csv) into **Nozomi Guardian/CMC assertions** — so the detections in this repo run natively in a Nozomi deployment, not just in a SIEM.
...
- Encrypted protocols (OPC UA SignAndEncrypt, MQTT over TLS) hide payload from DPI — those queries catch the *connection*; pair with server/broker logs for payload-level detection (see [`../docs/log-sources.md`](../docs/log-sources.md)).
