...
N2QL queries that operationalize the [detection catalog](../detections/detection-catalog.csv) inside Nozomi Guardian/CMC. Each is written in the style of Nozomi's *20 Queries* white paper and is designed to be saved as an **assertion** so it raises an alert when the condition is met.
